...
在账号详情的“重置次数”列表里，每张可用重置卡都有“使用”按钮：点击后网关会调用 `rate-limit-reset-credits/consume` 消耗选中的那张卡，重置成功后以服务器返回为准重新刷新额度和重置次数。

定时刷新会同时刷新订阅账号额度和配置了余额查询方式的 API 渠道余额；未配置余额查询方式的渠道会被自动跳过。
...
“适配远程压缩”用于解决第三方模型可能不支持远程压缩的问题。已支持原生压缩的渠道可以关闭该开关。

Codex 自动审查（auto review）只会发送固定的 `codex-auto-review` 模型 ID。可在“设置中心 > 账号与额度”中为它选择一个第三方渠道模型：当订阅账号池没有可用额度时，网关会把自动审查请求转给该渠道模型；留空则在账号池不可用时直接失败。

默认情况下，当 Codex 配置的当前模型只支持 HTTP 时，网关会在新建 WebSocket 连接时直接返回 426，让 Codex 立即改用 HTTP，而不是等第一条请求才降级。可在“设置中心 > 网络”中关闭“WS 握手拒绝 HTTP-only 模型”。
